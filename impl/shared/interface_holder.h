...
constexpr interface_type default_interface_type = interface_type::stack;
...
template <class T>
T *operator&(std::unique_ptr<T, detail::delete_interface<T>> &ptr)
{
    return ptr.get();
}

template <class T>
T *operator&(std::unique_ptr<T, detail::destroy_interface<T>> &ptr)
{
    return ptr.get();
}
...
struct construct_interface<interface_type::heap, T, false>
{
    ...
    using holder_type  = std::unique_ptr<T, detail::delete_interface<T>>;
...
{
    using element_type = T;
    using holder_type  = std::unique_ptr<T, detail::destroy_interface<T>>;

    template <typename... Args>
    static holder_type get(Args &&...args)
    {
        static uint8_t buffer[sizeof(element_type)];
...
#define FD_CONSTRUCT_INTERFACE(_T_, _IFC_)                                             \
    ...
    {                                                                                  \
        ...
        using holder_type  = std::unique_ptr<_IFC_, detail::delete_interface<_IFC_>>;  \
    ...
    {                                                                                  \
        using element_type = _IFC_;                                                    \
        using holder_type  = std::unique_ptr<_IFC_, detail::destroy_interface<_IFC_>>; \
        static holder_type get();                                                      \
    };
...
#define FD_CONSTRUCT_INTERFACE_IMPL(_T_)                                      \
    ...
    auto construct_interface<interface_type::stack, _T_>::get()->holder_type  \
    {                                                                         \
