#include "tier1/algorithm/find_xref.h"
...
template <class Lib = system_library_info>
static void* find_rtti_descriptor(string_view const name, void* image_base, typename Lib::sections_range sections)
{
    void* ret;
    if (auto const space = name.find(' '); space == name.npos)
    {
        ret = find<Lib>(image_base, sections, make_pattern(".?A", 1, name, 0, "\"@@\""));
    }
    else
    {
        ...
        array<char, 64> buff;
        ret = find<Lib>(image_base, sections, buff.begin(), fmt::format_to(buff.begin(), ".?A{}{}@@", object_tag, class_name));
    }
    return ret;
}
...
void* system_library_info::vtable(string_view name) const
{
    ...
    auto const nt_header  = this->nt_header();
    auto const image_base = this->image_base(nt_header);

    sections_range const sections(nt_header);
    ...
    section_view const rdata{find(sections, ".rdata"), image_base};
    ...
    section_view const text(find(sections, ".text"), image_base);
...
system_library_info literals::operator""_dll(wchar_t const* name, size_t length)
{
    return {
        {name, length},
        system_library_info::extension_tag::dll
    };
}
} // namespace FD_TIER(2)
