...
console::~console( )
{
	if(data_.console_allocated)
	{
		FreeConsole( );
		PostMessage(data_.console_window, WM_CLOSE, 0U, 0L);
	}
}

bool console::load_impl( )
{
	data_.console_window = GetConsoleWindow( );
	if(data_.console_window != nullptr)
	{
		data_.console_allocated = false;
	}
	else
	{
	...
	runtime_assert(IsWindowUnicode(data_.console_window) == true);
