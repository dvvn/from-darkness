...
void service_impl::remove_service(const std::type_info& info)
{
	...
	{
		auto dummy = loader->find_service<dummy_service>( );
		if (!dummy)
		{
			dummy         = loader->wait_for_service<dummy_service>( );
			const auto ex = loader->get_executor(1);
			[[maybe_unused]]
					const auto val = sync_wait(dummy->load(*ex));
			runtime_assert(val == true);
		}
		*itr = std::move(dummy);
	}
}
