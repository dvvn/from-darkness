...
module cheat.d3d_device9;
import cheat.application_info;
import cheat.rt_modules;

CHEAT_OBJECT_IMPL(IDirect3DDevice9, d3d_device9_raw, d3d_device9);

static auto _Find_d3d_device9(IDirect3DDevice9* known_ptr = nullptr)
{
    if (!known_ptr)
        known_ptr = cheat::runtime_modules::shaderapidx9.find_interface_sig<"A1 ? ? ? ? 50 8B 08 FF 51 0C">().plus(1).deref<2>();
