...
using namespace cheat::utl::hooks::detail;
using namespace cheat::utl::hooks;

bool detail::_Is_code_padding(LPBYTE pInst, UINT size)
{
...
bool detail::_Is_address_readable(LPVOID addr, size_t mem_size)
{
	return mem::memory_block(addr, mem_size).readable_ex( );
}

bool detail::_Is_address_executable(LPVOID addr, size_t mem_size)
{
	return mem::memory_block(addr, mem_size).executable( );
}
