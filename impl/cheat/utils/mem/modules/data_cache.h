...
#include "cheat/utils/mem/address.h"

namespace cheat::utl::mem
{
	enum class data_cache_result
	{
		unknown,
		success,
		error,
	...
	namespace detail
	{
		...
		public:
			...
			data_cache_result load( );
			data_cache_result load_from_memory( );
		...
		protected:
			...
			virtual data_cache_result Load_from_memory_impl( ) = 0;
		...
		protected:
			...
			data_cache_result Load_from_file(data_cache_base& vtable1, const path_type& full_path);
			data_cache_result Write_to_file(const data_cache_base& vtable1, const filesystem::path& full_path) const;

			virtual data_cache_result Write_to_file_impl(ptree_type& cache) const = 0;
			virtual data_cache_result Load_from_file_impl(const ptree_type& cache) = 0;

			data_cache_result Load(data_cache_base& vtable1, const path_type& full_path);
		};
		...
		public:
			...
			data_cache_result load_from_file(const path_type& full_path)
			{
				return data_cache_from_file::Load_from_file(*this, full_path);
			}

			data_cache_result write_to_file(const path_type& full_path) const
			{
				return data_cache_from_file::Write_to_file(*this, full_path);
			}

			data_cache_result load(const path_type& full_path)
			{
