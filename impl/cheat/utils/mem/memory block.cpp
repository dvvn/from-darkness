...
public:
	optional<bool> check_flags(SIZE_T block_size) const
	{
		...
		if (utl::invoke(check_fn, this->get_flags( ), flags_checked__) == false)
			return false;
...
constexpr auto PAGE_READ_FLAGS = mem_flags_t(PAGE_READONLY, PAGE_READWRITE, PAGE_EXECUTE_READ, PAGE_EXECUTE_READWRITE).get( );
constexpr auto PAGE_WRITE_FLAGS = mem_flags_t(PAGE_READWRITE, PAGE_WRITECOPY, PAGE_EXECUTE_READWRITE, PAGE_EXECUTE_WRITECOPY, PAGE_WRITECOMBINE).get( );
constexpr auto PAGE_EXECUTE_FLAGS = mem_flags_t(PAGE_EXECUTE, PAGE_EXECUTE_READ, PAGE_EXECUTE_READWRITE, PAGE_EXECUTE_WRITECOPY).get( );
