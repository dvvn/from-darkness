							  ...
							  , smooth_value_base<ImVec4>* bg_animation, smooth_value_base<ImVec4>* check_animation)
{
	...
	const auto square_sz = text.label_size.y + style.FramePadding.y * 2.0f;
	const auto check_pos = window->DC.CursorPos;

	const auto check_bb  = ImRect(check_pos, check_pos + ImVec2(square_sz, square_sz));
	const auto label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + text.label_size.y * 0.25f/*+ style.FramePadding.y*/);
	const auto label_bb  = ImRect(label_pos, label_pos + text.label_size);
	...
	const auto id    = imgui_id(text, window);
	const auto state = text_clickable ? button_behavior(total_bb, id) : button_behavior(check_bb, id, render_check == false);
	...
	if (render_check)
	{
		...
		{
			const auto pad = std::max(1.0f, std::floor(square_sz / 6.0f));
			ImGui::RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_color, square_sz - pad * 2.0f);
		}
	}

	if (render_label)
		text.render(window->DrawList, label_pos, ImGui::GetColorU32(ImGuiCol_Text));
