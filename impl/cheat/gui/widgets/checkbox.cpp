...
button_state widgets::checkbox2(const cached_text& label,/* nstd::confirmable_value<bool>*/bool& value, smooth_value_base<ImVec4>* bg_animation
							  , smooth_value_base<ImVec4>* check_animation)
{
	const auto window = ImGui::GetCurrentWindow( );
	const auto& style = ImGui::GetStyle( );

	const auto square_sz = label.get_font( )->FontSize + style.FramePadding.y * 2.0f;
	...
	const auto total_bb  = ImRect(check_bb.Min, label_bb.Max);
	...
	if (render_check)
	{
		...
		{
			const auto pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
