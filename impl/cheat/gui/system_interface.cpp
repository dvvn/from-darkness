...
system_interface::system_interface() : start_time_(clock::now())
{
}
...
{
    logger_system_console->log("[RmlUi] {}", [&] { return std::vformat(str, std::make_format_args(std::forward<Args>(args)...)); });
}
...
#endif
    case Log::LT_ALWAYS:
        if (!logger_system_console.initialized() || !logger_system_console->active())
