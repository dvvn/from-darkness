...
class native_dx11_backend final : basic_dx11_backend, public native_dx11_backend_protector, public noncopyable
{
    friend struct native_dx11_backend_protector;
  ...
  public:
    native_dx11_backend(pointer device)
        : basic_dx11_backend(*device)
        , device_(device)
    {
    }

    native_dx11_backend(system_library_info info)
        : native_dx11_backend([info] {
            assert(info.name() == L"shaderapidx11.dll");
...
namespace detail
{
struct simple_imgui_dx11_data
{
    IDirect3DDevice9* device;
};
} // namespace detail

native_dx11_backend_protector::~native_dx11_backend_protector()
{
