...
native_dx11_device_data::native_dx11_device_data(system_library_info info)
{
    ...
    auto const addr = info.pattern("66 0F 7F 05 ? ? ? ? 66 0F 7F 0D ? ? ? ? 48 89 35"_pat);
    assert(addr != nullptr);
...
{
    auto const result = data_.device->CreateRenderTargetView(back_buffer, target_view_desc, &render_target_);
...
native_dx11_backend::native_dx11_backend(native_dx11_device_data data)
    : basic_dx11_backend(data.device, data.device_context)
    , data_(std::move(data))
{
