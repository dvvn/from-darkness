...
template <class T>
class cahced_entity<T, false>
{
    T ent_;
    bool removed_;

  public:
    cahced_entity(native::entity_instance* ent)
        : ent_{ent}
        , removed_{true}
    {
    }
...
class cahced_entity<T, true>
{
    ...
    class entity_view
    {
      ...
      public:
        ...
        entity_view(cahced_entity* self)
            : self_{self}
        {
            ++self_->use_count_;
        }
...
class entity_cache : public noncopyable
{
    ...
    using cached_player = cahced_entity<player, store_removed_players>;
