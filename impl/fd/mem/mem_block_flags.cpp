...
static bool _Flags_iterator(std::span<uint8_t> mblock, Fn func = {})
{
    for (;;)
    {
        ...
        if (info.size() >= mblock.size())
            return true;
        // check next mem_block
        mblock = mblock.subspan(info.size());
    }
}

static bool _Have_flags(const mem_block* mblock, DWORD flags)
{
    return _Flags_iterator(*mblock, [flags](const DWORD mem_flags) {
        return !!(mem_flags & flags);
    });
}

static bool _Dont_have_flags(const mem_block* mblock, DWORD flags)
{
    return _Flags_iterator(*mblock, [flags](const DWORD mem_flags) {
        return !(mem_flags & flags);
    });
...
bool mem_block::have_flags(DWORD flags) const
{
    return _Have_flags(this, flags);
}

bool mem_block::dont_have_flags(DWORD flags) const
{
    return _Dont_have_flags(this, flags);
}

DWORD mem_block::get_flags() const
{
    DWORD flags       = 0;
