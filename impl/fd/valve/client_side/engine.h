...
class engine
{
  ...
  public:
    ...
    virtual int GetIntersectingSurfaces(
        ...
        float radius,
        bool OnlyVisibleSurfaces,
        SurfInfo *pInfos,
        int nMaxInfos)                                                 = 0;
    virtual vector3d GetLightForPoint(const vector3d &pos, bool Clamp) = 0;
    virtual material *TraceLineMaterialAndLighting(
        const vector3d &start,
        const vector3d &end,
        vector3d &diffuseLightColor,
        vector3d &baseColor)                                                                          = 0;
    ...
    virtual int IsBoxVisible(const vector3d &mins, const vector3d &maxs)                              = 0;
    virtual int IsBoxInViewCluster(const vector3d &mins, const vector3d &maxs)                        = 0;
    virtual bool CullBox(const vector3d &mins, const vector3d &maxs)                                  = 0;
    ...
    virtual void ComputeDynamicLighting(const vector3d &pt, const vector3d *pNormal, vector3d &color) = 0;
    virtual void GetAmbientLightColor(vector3d &color)                                                = 0;
    ...
    virtual bool DoesBoxTouchAreaFrustum(const vector3d &mins, const vector3d &maxs, int iArea)       = 0; // 60
    ...
    virtual int IsSkyboxVisibleFromPoint(const vector3d &vecPoint)               = 0;
    ...
    virtual vector3d GetLightForPointFast(const vector3d &pos, bool Clamp)       = 0;
    ...
    virtual void ComputeLightingCube(const vector3d &pt, bool Clamp, vector3d *pBoxColors)                         = 0;
    ...
    virtual void SetMixGroupOfCurrentMixer(
    ...
    virtual const char *Key_LookupBindingEx(
        const char *pBinding,
        int iUserId        = -1,
        int iStartCount    = 0,
    ...
    virtual void SolidMoved(
        client_entity *pSolidEnt,
        collideable *pSolidCollide,
        const vector3d *pPrevAbsOrigin,
        bool accurateBboxTriggerChecks)                                                   = 0;
    virtual void TriggerMoved(client_entity *pTriggerEnt, bool accurateBboxTriggerChecks) = 0;
    virtual void ComputeLeavesConnected(
        const vector3d &vecOrigin,
        int nCount,
    ...
    virtual void SpherePaintSurface(
        const model_t *model,
    ...
    // virtual void                TracePaintSurface( const model_t *model, const vector3d& position, float radius,
    // vecor<color>& surfColors ) = 0;
    virtual void SphereTracePaintSurface(
        const model_t *model,
        const vector3d &position,
        const vector3d &vec2,
        float radius,
        /*vecor<unsigned char, valve_memory<unsigned char, int>>*/ int &utilVecShit) = 0;
