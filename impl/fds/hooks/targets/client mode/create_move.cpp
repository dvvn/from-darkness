...
module fds.hooks.client_mode.create_move;
import fds.csgo.interfaces.Prediction;
import fds.csgo.interfaces.EngineClient;
import fds.csgo.interfaces.ClientMode;
...
FDS_HOOK(create_move, member){create_move_impl(){this->init({&instance_of<ClientModeShared>, 24}, &create_move_impl::callback);
}
