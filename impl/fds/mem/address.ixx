...
#define ADDR_EQ_OP(_OP_)                                    \
    ...
    auto FDS_CONCAT(operator, _OP_)(const Q other) const    \
    {                                                       \
...
#define ADDR_MATH_OP(_OP_, _NAME_)                                                                     \
    template <constructible_from<basic_address> Q>                                                     \
    basic_address& FDS_CONCAT(operator, FDS_CONCAT(_OP_, =))(const Q other)                            \
    {                                                                                                  \
        static_assert(!std::is_class_v<value_type>, __FUNCSIG__ ": unable to change the class type!"); \
        this->value FDS_CONCAT(_OP_, =) basic_address<void>(other).value;                              \
        return *this;                                                                                  \
    }                                                                                                  \
    template <constructible_from<basic_address> Q>                                                     \
    basic_address<safe_out_type> FDS_CONCAT(operator, _OP_)(const Q other) const                       \
    {                                                                                                  \
