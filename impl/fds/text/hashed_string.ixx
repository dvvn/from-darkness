...
export module fds.hashed_string;
...
struct hashed_string_wrapper : Base
{
  ...
  public:
    ...
    requires(std::constructible_from<Base, Base2>) constexpr hashed_string_wrapper(const hashed_string_wrapper<Base2, Hasher2>& other)
    ...
    requires(std::constructible_from<Base, Base2>) constexpr hashed_string_wrapper(hashed_string_wrapper<Base2, Hasher2>&& other)
    ...
    requires(std::constructible_from<Base, Base2>) constexpr hashed_string_wrapper(Base2&& other)
    ...
    requires(std::constructible_from<Base, Base2>) constexpr hashed_string_wrapper(Base2&& other, hash_type hash)
        : Base(std::forward<Base2>(other))
    {
        _Write_hash(hash);
    }
...
#define OPERATOR_IMPL(_CHECK_, _RET_, _OP_)                                                                                     \
    ...
    {                                                                                                                           \
        ...
        static_assert(fds::same_template<Hasher, Hasher2>(), __FUNCSIG__ ": Hash functions must be have same base!");           \
