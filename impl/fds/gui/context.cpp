...
FDS_OBJECT_BIND(Rml::Context*, context, gui_context);
...
{
    const auto size = fds::app_info->window.size.client();
...
gui_context::gui_context()
{
    Rml::SetRenderInterface(&FDS_OBJECT_GET(Rml::RenderInterface));
    Rml::SetSystemInterface(&FDS_OBJECT_GET(Rml::SystemInterface));
