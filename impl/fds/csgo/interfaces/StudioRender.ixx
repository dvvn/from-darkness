...
export module fds.csgo.interfaces.StudioRender;
export import fds.csgo.interfaces.Studio;
export import fds.csgo.interfaces.AppSystem;
export import fds.csgo.interfaces.ModelRender;
...
export namespace fds::csgo
{
    ...
    class IStudioRender : public IAppSystem
    {
      public:
        ...
        // Draws the model
        virtual void DrawModel(DrawModelResults_t* pResults, const DrawModelInfo_t& info, math::matrix3x4* pBoneToWorld, float* pFlexWeights, float* pFlexDelayedWeights, const math::vector3& modelOrigin, DrawModelFlags_t flags = DrawModelFlags_t::DRAW_ENTIRE_MODEL) = 0;
        ...
        virtual void AddDecal(StudioDecalHandle_t handle, studiohdr_t* pStudioHdr, math::matrix3x4* pBoneToWorld, const Ray_t& ray, const math::vector3& decalUp, IMaterial* pDecalMaterial, float radius, int body, bool noPokethru = false, int maxLODToDecal = /*ADDDECAL_TO_ALL_LODS*/ -1, void* pvProxyUserData = nullptr, int nAdditionalDecalFlags = 0) = 0;

        // Compute the lighting at a point and normal
        virtual void ComputeLighting(const math::vector3* pAmbient, int lightCount, LightDesc_t* pLights, const math::vector3& pt, const math::vector3& normal, math::vector3& lighting) = 0;

        // Compute the lighting at a point, constant directional component is passed
        // as flDirectionalAmount
        virtual void ComputeLightingConstDirectional(const math::vector3* pAmbient, int lightCount, LightDesc_t* pLights, const math::vector3& pt, const math::vector3& normal, math::vector3& lighting, float flDirectionalAmount) = 0;

        // Shadow state (affects the models as they are rendered)
        virtual void AddShadow(IMaterial* pMaterial, void* pProxyData, FlashlightState_t* m_pFlashlightState = nullptr, math::view_matrix* pWorldToTexture = nullptr, ITexture* pFlashlightDepthTexture = nullptr) = 0;
        ...
        virtual void DrawModelArray(const StudioModelArrayInfo_t& drawInfo, int nCount, StudioArrayInstanceData_t* pInstanceData, int nInstanceStride, DrawModelFlags_t flags = DrawModelFlags_t::DRAW_ENTIRE_MODEL) = 0;
        ...
        // draw an array of models with the same state
        virtual void DrawModelArray(const StudioModelArrayInfo2_t& info, int nCount, StudioArrayData_t* pArrayData, int nInstanceStride, DrawModelFlags_t flags = DrawModelFlags_t::DRAW_ENTIRE_MODEL) = 0;
