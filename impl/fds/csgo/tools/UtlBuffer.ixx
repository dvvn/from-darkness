export module fds.csgo.tools.UtlBuffer;
export import fds.csgo.tools.UtlMemory;
...
#define IsX360() (0)
	...
	public:
		...
		//-----------------------------------------------------------------------------
		inline bool IsSwappingBytes( )	// Are bytes being swapped?
		{
			return m_bSwapBytes;
		}

		inline bool IsTargetBigEndian( )	// What is the current target endian?
		{
			return m_bBigEndian;
		}
		...
		// If inputBuffer is omitted or 0, then it is assumed to be the same as
		// outputBuffer - effectively swapping the contents of the buffer in place.
		//-----------------------------------------------------------------------------
		template<typename T> inline void SwapBuffer(T* outputBuffer, T* inputBuffer = 0, int count = 1)
		{
			assert(count >= 0);
			assert(outputBuffer);
		...
		// outputBuffer - effectively swapping the contents of the buffer in place.
		//-----------------------------------------------------------------------------
		template<typename T> inline void SwapBufferToTargetEndian(T* outputBuffer, T* inputBuffer = 0, int count = 1)
		{
			assert(count >= 0);
			assert(outputBuffer);
...
#define BEGIN_CUSTOM_CHAR_CONVERSION(_className, _name, _delimiter, _escapeChar) static CUtlCharConversion::ConversionArray_t s_pConversionArray##_name[] = {
...
//-----------------------------------------------------------------------------
	...
	public:
		...
		__forceinline void ActivateByteSwappingIfBigEndian( )
		{
			if (IsX360( ))
	...
	public:
		...
		//
		// If end of file is reached or upon error returns 0.
		//
		// Note:	the pointer returned points into the local memory of this buffer, in
		//			case the buffer gets relocated or destroyed the pointer becomes invalid.
		//
		...
		//			0							when EOF is reached or error occurs
		//
