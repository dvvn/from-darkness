...
export module fds.csgo.tools.UtlVector;
...
inline int CUtlVector<T, A>::AddToHead(const T& src)
{
    ...
    assert((Base() == 0) || (&src < Base()) || (&src >= (Base() + Count())));
...
{
    // Can't insert something that's in the list... reallocation may hose us
    assert((Base() == 0) || !pToCopy || (pToCopy + num <= Base()) || (pToCopy >= (Base() + Count())));
...
{
    // Can't insert something that's in the list... reallocation may hose us
    assert((Base() == 0) || !pArray || (Base() >= (pArray + size)) || (pArray >= (Base() + Count())));
