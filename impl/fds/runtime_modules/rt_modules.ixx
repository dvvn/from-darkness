...
class interface_finder
{
  ...
  public:
    ...
    {
        T* const ptr = addr_;
        console_log(rt_module_._Name(), "interface", fds::object_name<T>, ptr);
        return ptr;
    }
...
struct rt_module
{
    ...
    T* find_vtable() const
    {
        static const auto found = wp::find_vtable<logs_writer>(wp::find_module<Name, logs_writer>(), this->_Name(), fds::object_name<T>);
...
#define FDS_GAME_MODULE(_NAME_) constexpr rt_module<DLL_NAME(_NAME_)> _NAME_;

export namespace fds::runtime_modules
{
    constexpr current_module current;

    FDS_GAME_MODULE(server);
    FDS_GAME_MODULE(client);
    FDS_GAME_MODULE(engine);
    FDS_GAME_MODULE(datacache);
    FDS_GAME_MODULE(materialsystem);
    FDS_GAME_MODULE(vstdlib);
    FDS_GAME_MODULE(vgui2);
    FDS_GAME_MODULE(vguimatsurface);
    FDS_GAME_MODULE(vphysics);
    FDS_GAME_MODULE(inputsystem);
    FDS_GAME_MODULE(studiorender);
    FDS_GAME_MODULE(shaderapidx9);
} // namespace fds::runtime_modules
