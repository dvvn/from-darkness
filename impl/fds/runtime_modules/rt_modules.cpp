...
{
    fds::logger_system_console->log(L"{} -> {} \"{}\" {}", _Bind_front<extract_module_name>(module_name), object_type, object_name, _Bind_front<inform_found_pointer>(object_ptr));
}
...
{
    fds::logger_system_console->log(L"module \"{}\" found at {:#X}", module_name, reinterpret_cast<uintptr_t>(ldr_entry));
}
